...
CREATE POLICY "Enable update for fund custodians"
  ON petty_cash.transactions FOR UPDATE
  TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM petty_cash.funds
...
CREATE POLICY "Enable delete for fund custodians"
  ON petty_cash.transactions FOR DELETE
  TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM petty_cash.funds
...
CREATE POLICY "Enable update for fund custodians and admins"
  ON petty_cash.reconciliations FOR UPDATE
  TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM petty_cash.funds f
      WHERE f.id = fund_id AND (f.custodian_id = auth.uid() OR
        EXISTS (
          SELECT 1 FROM public.user_profiles
          WHERE id = auth.uid() AND role = 'admin'
        )
...
CREATE INDEX idx_funds_custodian ON petty_cash.funds(custodian_id);
CREATE INDEX idx_transactions_fund ON petty_cash.transactions(fund_id);
CREATE INDEX idx_transactions_user ON petty_cash.transactions(user_id);
CREATE INDEX idx_reconciliations_fund ON petty_cash.reconciliations(fund_id);
CREATE INDEX idx_reconciliations_user ON petty_cash.reconciliations(reconciled_by);

-- Create or replace fund summary view
